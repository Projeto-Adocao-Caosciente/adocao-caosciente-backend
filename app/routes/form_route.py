...
async def create_form(request: Request, animal_id, form: FormModel = Body(..., example=FormModel.Config.json_schema_extra)):
    ...
    receveid_fields = set([ key for key, value in form.model_dump().items() if value is not None])
...
async def read_forms_from_animal(
    ...
    request_id = request.state.request_id
    ong_id = jwt_bearer.get_user_id()
    response = form_service.get_forms_from_animal(
        animal_id, ong_id, request_id)
    return JSONResponse(
        status_code=response.status,
        content=response.dict()
    )
